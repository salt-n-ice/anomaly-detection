...
def _minimal_viz_scenario():
    ...
    events = _pd.DataFrame({
        ...
        "sensor_id": (["outlet_tv_power"]*6 + ["basement_leak"]*4),
        "capability": (["power"]*6 + ["water"]*4),
        "value": [100.0, 110.0, 250.0, 240.0, 105.0, 102.0,
                  0, 0, 1, 0],
        "unit": ["W"]*6 + [""]*4,
    })
    labels = _pd.DataFrame({
        "sensor_id": ["outlet_tv_power", "basement_leak"],
        "capability": ["power", "water"],
        ...
        "anomaly_type": ["weekend_anomaly", "water_leak_sustained"],
